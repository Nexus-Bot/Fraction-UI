      ...
      {navOptions.map(([title, url, active]) => (
        <a
          ...
          className={`px-3 py-3 text-l font-semibold ${
            active
              ? "text-emerald-900 text-slate-700 hover:text-slate-900 hover:font-bold"
              : "rounded text-zinc-400 cursor-not-allowed disabled"
      ...
      {navOptions.map(([title, url, active]) =>
        active ? (
          <a
            key={title}
            href={active ? url : ""}
            className={`rounded-[16px] text-white px-4 py-2 md:px-6 md:py-3 text-sm md:text-base ${
              active
                ? "hover:text-gray-100 text-white hover:text-slate-900"
                : "rounded text-zinc-400 cursor-not-allowed disabled"
            } ${
              pageLoad === title ? "font-semibold bg-stiletto-400" : "font-base"
            }`}
          >
            {title}
          </a>
      ...
      <div className="flex flex-col pt-20 pl-8">
        {navOptions.map(([title, url, active]) => (
          <a
            key={title}
            href={active ? url : ""}
            className="text-2xl md:text-4xl md:m-5 text-greenKelp-500 hover:text-greenKelp-100 m-1 font-bold"
          >
            {title}
          </a>
        ))}
  ...
  return (
    <>
      <div className="fixed z-50">
        {burgerMenu === true ? (
          <BurgerMenu setBurgerMenu={setBurgerMenu} pageLoad={pageLoad} />
        ) : (
          ""
        )}
      </div>
      <div className="lg:invisible fixed flex flex-row justify-center bottom-0 mb-4 w-full">
        {pageLoad !== "Default" ? (
          <PopulateBottomNav pageLoad={pageLoad} />
        ) : (
          ""
        )}
      </div>
      <div className="p-2 fixed top-0 border-b-2 border-emerald-900 bg-gin-100 w-full z-20 h-content">
        <nav className="flex items-center space-x-6">
              ...
              <div>
                {burgerMenu !== true ? (
                  <img
                    ...
                    src="hamburger.png"
                    className="mr-6 w-8 h-6 lg:w-8 lg:h-8 md:mr-4"
                  />
                ) : (
                  ""
                )}
          ...
          {!["Default", "Trade"].includes(pageLoad) ? (
            wallet.loading ? (
              <button
