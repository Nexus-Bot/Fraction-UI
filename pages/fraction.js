...
const executeFractionalisation = async ({
  walletContext,
  contractAddress,
  tokenID,
  tokenCount,
  ...
  if (tokenApproved.toLowerCase() !== FRACTION_CONTRACT_ADDRESS.toLowerCase()) {
    const txnReceipt = await signedTokenAddress.approve(
      FRACTION_CONTRACT_ADDRESS,
      tokenID
    );
    ...
    const txnReceipt = await signedStakingContract.fractionalize(
      contractAddress,
      tokenID,
      tokenCount
    );
...
const FractionCard = ({ walletNFTsList = [], walletContext }) => {
  ...
  const fractionalise = ({ key, contractAddress, tokenID }) => {
    ...
    if (tokenCount <= 0 || tokenCount > maxFractionCount) {
      <div
        className="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg dark:bg-red-200 dark:text-red-800 z-50"
        role="alert"
      >
        <span className="font-medium">
          Fraction Count should be less than {MAX_FRACTION_COUNT}
        </span>
      </div>;
    } else if (walletContext) {
      executeFractionalisation({
        walletContext,
        contractAddress,
        tokenID,
        tokenCount,
                  ...
                  <div className="px-4 py-2 lg:py-4">
                    ...
                    <div className="flex flex-row">
                      ...
                      <input
                        ...
                        className="min-w-0 block w-20 lg:w-28 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-emerald-900 focus:outline-none"
                        type="number"
                        name="fractionCount"
                        required
                      />
                  ...
                  <div className="h-full w-full">
                    <button
                      onClick={() =>
                        fractionalise({
                          key: data.originalAddress + "-" + data.tokenID,
                          contractAddress: data.originalAddress,
