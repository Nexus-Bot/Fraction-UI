  ...
  return (
    <>
      <Navbar
        className="invisible"
        pageLoad="Faucet"
        setWalletContext={setWalletContext}
      />
      <div className="w-full h-screen bg-gin-50 flex flex-row justify-center items-center">
        <div className="flex flex-col justify-center p-10">
          <button
            onClick={() => mintNFT()}
            className={`text-semibold lg:px-8 py-3 lg:py-4 my-2 lg:my-4 text-xl md:text-2xl lg:text-3xl + ${
              walletConnected
                ? "bg-greenKelp-300 text-white hover:bg-greenKelp-400"
                : "bg-gray-500 text-white cursor-not-allowed disabled"
            }`}
          >
            Mint
          </button>
          <div className="text-semibold text-base lg:text-xl">
            Supply Left: {supply}{" "}
          </div>
        </div>
      </div>
