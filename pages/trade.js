...
const Trade = () => {
  ...
  const fetchAllFractionData = async () => {
    ...
    let response = await fetch(
      "https://api.thegraph.com/subgraphs/name/cpp-phoenix/fraction",
    ...
    if (response.status === 200) {
      ...
      data.data.tokens.map((token) => {
        fractionData.push({
          ...
          openSeaLink:
            OPENSEA_LINK + token.fractionContract.id + "/" + token.id,
        });
      });
    }
...
//     let response = await fetch("https://api.thegraph.com/subgraphs/name/cpp-phoenix/fraction", requestOptions(FETCH_ALL_NFTs));
